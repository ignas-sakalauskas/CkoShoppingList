...
namespace Checkout
{
    public sealed class APIClient
    {
        ...
        public ChargeService ChargeService => _chargeService ?? (_chargeService = new ChargeService());
        public CardService CardService => _cardService ?? (_cardService = new CardService());
        public CustomerService CustomerService => _customerService ?? (_customerService = new CustomerService());
        public TokenService TokenService => _tokenService ?? (_tokenService = new TokenService());
        public ReportingService ReportingService => _reportingService ?? (_reportingService = new ReportingService());
        public LookupsService LookupsService => _lookupsService ?? (_lookupsService = new LookupsService());
        public RecurringPaymentsService RecurringPaymentsService => _recurringPaymentsService ?? (_recurringPaymentsService = new RecurringPaymentsService());
        public ShoppingListService ShoppingListService => _shoppingListService ?? (_shoppingListService = new ShoppingListService());
